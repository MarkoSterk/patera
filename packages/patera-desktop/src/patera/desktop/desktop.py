...
class Desktop(BaseExtension[AppT, DesktopConfig], Generic[AppT]):
    ...
    def evaluate_js(
        ...
        *,
        ...
        use_window: webview.Window = window if window else self.main_window
        use_window.evaluate_js(code, callback=callback)
    ...
    def remove_active_window(self, window: webview.Window) -> None:
        self._active_windows.remove(window)
        window.destroy()

    def go_home(self) -> None:
        self._window.load_url(self.configs.HOME_URL)
    ...
    def icon(self) -> str:
        """
        Override this method and return a string path to the icon
        """
        ...
