...
from patera.controller import Controller, get, post
...
class _AdminDbController(Controller[Patera]):
    ...
    async def create_record(
        ...
        # Adjust this line if your Request API exposes form data differently.
        form_data = await req.form()
        new_data = dict(form_data)

        new_data = self.normalize_create_form_data(
            new_data=new_data,
            create_fields=create_fields,
        )
        ...
        async with db.create_session() as session:
            record = model()
            await record.admin_create(req, new_data, session)
    ...
    def get_admin_create_form_fields(
        ...
        The returned list is intentionally template-friendly.
        """
        mapper = inspect(model)
    ...
    def normalize_create_form_data(
        self,
        new_data: dict[str, Any],
        create_fields: list[dict[str, Any]],
    ) -> dict[str, Any]:
        """
        Normalizes form data before passing it to model.admin_create().

        Handles:
        - unchecked checkboxes
        - checked checkbox string values
        - empty strings as None for non-text optional fields
        """
        ...
        for field in create_fields:
            ...
            if input_type == "checkbox":
                normalized_data[field_name] = field_name in normalized_data
                continue
